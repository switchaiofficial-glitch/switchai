...
export default function SettingsPage() {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div style={{ marginBottom: 24 }}>
      <div style={{ color: '#cbd5e1', fontSize: 14, fontWeight: 800, letterSpacing: 0.4, padding: '0 4px', marginBottom: 12 }}>{title}</div>
  ...
  const Row = ({ icon, label, sub, onClick, danger, isLast }: { icon: React.ReactNode; label: string; sub?: string; onClick?: () => void; danger?: boolean; isLast?: boolean }) => (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 12px', cursor: onClick ? 'pointer' : 'default', borderBottom: isLast ? 'none' : `1px solid ${theme.colors.border}`, background: danger ? 'rgba(253, 164, 175, 0.08)' : 'transparent' }}>
    ...
    <div style={{ minHeight: '100dvh', background: `linear-gradient(180deg, ${theme.gradients.background.join(', ')})`, color: theme.colors.text, position: 'relative' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 16 }}>
        ...
        <Section title="Account">
          ...
          <Row icon={<Phone size={18}/>} label="Phone number" sub={phone} isLast />
        </Section>

        {/* Tokens */}
        <Section title="Tokens">
          <Row icon={<Rocket size={18}/>} label="Tokens" sub="View balance and usage" onClick={()=>navigate('/settings/tokens')} isLast />
        </Section>

        {/* App */}
        <Section title="App">
          <Row icon={<Brain size={18}/>} label="Personalization" sub="Models and preferences" onClick={()=>navigate('/settings/personalization')} />
          <Row icon={<Brain size={18}/>} label="AI Memory" sub="Personalized context and preferences" onClick={()=>navigate('/settings/ai-memory')} />
          ...
          <Row icon={<Info size={18}/>} label="About" onClick={()=>navigate('/settings/about')} isLast />
        </Section>
