...
import { isAutoSwitchEnabled, setAutoSwitchEnabled } from '../lib/autoModelSwitch';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const loadUserPlan = async () => {
      if (user?.uid) {
        try {
          const data = await fetchUserTier(user.uid);
          if (data?.plan) {
            setPlan(data.plan);
          }
        } catch (error) {
  ...
  const handleDragLeave = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    // Only set to false if we're leaving the main container completely
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const unsupportedFiles = files.filter(f => !f.type.startsWith('image/') && f.type !== 'application/pdf');

    // Show toast for unsupported files
    if (unsupportedFiles.length > 0) {
      const names = unsupportedFiles.map(f => f.name).slice(0, 3).join(', ');
      const suffix = unsupportedFiles.length > 3 ? ` and ${unsupportedFiles.length - 3} more` : '';
      toast.error(`Unsupported file format: ${names}${suffix}. Only images and PDFs are supported.`);
    }
    ...
    if (pdfFiles.length > 0) {
      // Check if already have a PDF attached
      if (attachedPDFs.length >= 1) {
        toast.error('You can only attach 1 PDF per message. Remove the existing PDF first.');
        return;
      }

      // Check if trying to drop multiple PDFs
      if (pdfFiles.length > 1) {
        toast.error('Please drop only 1 PDF file.');
        return;
      }
      ...
      if (validation.valid) {
        setAttachedPDFs([{ file, name: file.name }]);
        // Auto-check server health when PDFs are added
        serverHealthMonitor.forceCheck(ServerType.OCR_SERVER).then(health => {
          if (!health.isHealthy) {
            toast('Note: Smart Doc server is currently offline. Basic processing will be used.', {
              icon: '⚠️',
              duration: 4000,
            });
          }
    ...
    // Force UI update if we added anything
  ...
  const [playstoreAnimation, setPlaystoreAnimation] = useState<any>(null);
  const playstoreLottieRef = useRef<any>(null);
  ...
  useEffect(() => {
    const loadAnimations = async () => {
      try {
        ...
        const playstoreResponse = await fetch('/animations/playstore.json');
        const playstoreData = await playstoreResponse.json();
        setPlaystoreAnimation(playstoreData);
      } catch (error) {
  ...
  // Ensure playstore Lottie plays at desired speed when loaded
  useEffect(() => {
    if (!playstoreAnimation) return;
    try {
      // lottie-react exposes the animation instance via lottieRef.current
      if (playstoreLottieRef.current && typeof playstoreLottieRef.current.setSpeed === 'function') {
        playstoreLottieRef.current.setSpeed(0.4);
      }
    } catch (e) {
      // ignore
    }
  ...
  const isPillMode = !input.includes('\n') && inputHeight <= 75 && attachedImages.length === 0 && attachedPDFs.length === 0;
  ...
  const loadModels = async () => {
    try {
      ...
      // Fetch models from server API instead of Firestore
      const { fetchModels } = await import('../lib/unifiedApiClient');
      const serverModels = await fetchModels();

      // Convert server models to catalog format
      const modelData = serverModels.map(model => ({
        model: model.label,
        modelID: model.id,
        type: model.type,
        inference: model.inference,
        provider: model.provider, // Provider from server
      }));
  ...
  const handleImagePick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Force rectangular mode to show attachments
    setInputHeight(50);
  ...
  const handlePDFPick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check if trying to select multiple files
    if (files.length > 1) {
      toast.error('Please select only 1 PDF file.');
      e.target.value = ''; // Reset input
      return;
    }
    ...
    if (validation.valid) {
      ...
      toast.error(`${file.name}: ${validation.error || 'Invalid PDF file'}`);
    }
  ...
  const handleSend = async () => {
    ...
    if (!rateLimitCheck.allowed) {
      toast.error(`Rate limit exceeded. Please wait ${rateLimitCheck.retryAfter} seconds.`);
      return;
    }
    ...
    // Note: Auto-switching is now handled server-side
    // The server will analyze intent and select the best model when model='auto' is passed
    ...
    (async () => {
      ...
      // Handle PDF files using improved Smart Doc Tool (VivekTools)
      if (hasPDFs) {
        try {
          ...
          for (let i = 0; i < localPDFs.length; i++) {
            ...
            // Show progress toast
            const toastId = toast.loading(`Initializing Smart Doc scan for ${pdf.name}...`);

            // Use improved service with progress
            const result = await VivekToolsPDFService.extractTextWithProgress(
              pdf.file,
              (msg) => {
                // Update toast with progress ("Reading line 1-100..." style feedback)
                toast.loading(msg, { id: toastId });
              },
              { uid: user?.uid, chatId: chatId }
            );
            ...
            if (result.success && result.text) {
              ...
              // CRITICAL: Capture filePath for Smart Search tool
              // The AI server uses this path to read the OCR'd markdown file
              if (result.filePath) {
                prefaceParts.push(`[System: File Path: ${result.filePath}]`);
                console.log('[Smart Search] Captured filePath for tool:', result.filePath);
              }

              // Success feedback
              const pagesApprox = Math.ceil(result.text.length / 3000);
              toast.success(`Scanned ${pagesApprox} pages from ${pdf.name}`, { id: toastId });
            } else {
              // Detailed error handling
              if (result.serverDown) {
                toast.error(`OCR Server Unavailable. Trying fallback...`, { id: toastId });
                // Fallback to local logic or basic extraction if possible could go here, 
                // but mainly we just notify the user.
              } else if (result.usage?.chatLocked) {
                toast.error(`Smart Doc Limit Reached: ${result.usage.lockedReason}`, { id: toastId });
              } else {
                toast.error(`Failed to process ${pdf.name}: ${result.error}`, { id: toastId });
              }
            }
          }
          if (pdfTexts.length > 0) {
            prefaceParts.push(`PDF context (Extracted via Smart Doc Tool):\n${pdfTexts.join('\n\n---\n\n')}`);
          }
        } catch (error) {
          console.warn('PDF OCR failed', error);
          toast.error('Failed to process PDF. Smart Doc Tool encountered an error.');
        }
      }
      ...
      // Update the user message for DISPLAY (NOT the full OCR text)
      // Show user's text + indicator that files were processed
      let displayContent = userText || '';
      if (hasPDFs && prefaceParts.length > 0) {
        // Add a small indicator that PDFs were processed, but NOT the full OCR text
        displayContent = displayContent || '📎 Document attached';
      }

      // Extract just the filePath for storage (for follow-up questions)
      const filePathMatch = prefaceParts.find(p => p.includes('[System: File Path:'));
      const extractedFilePath = filePathMatch?.match(/\[System: File Path: ([^\]]+)\]/)?.[1];

      const updatedUserMessage = {
        ...userMessage,
        content: displayContent,
        // Store filePath as metadata for follow-up questions
        ...(extractedFilePath && { pdfContext: { filePath: extractedFilePath } }),
      };
      ...
      if (currentChat) {
        const updatedStoredMessages = currentChat.messages.map(msg =>
          msg.id === userMessage.id ? updatedUserMessage : msg
        );
        updateMessages(chatId, updatedStoredMessages);
      }
  ...
  const sendToAPI = async (chatId: string, updatedMessages: Message[], apiUserContent: any, userText: string) => {
    ...
    try {
      ...
      // Check for PDF context in previous messages (for follow-up questions)
      // This injects [System: File Path:...] from stored pdfContext metadata
      let pdfContextPrefix = '';

      // First, check the passed messages
      for (const msg of compressedHistory) {
        const msgWithContext = msg as any;
        if (msgWithContext.pdfContext?.filePath) {
          pdfContextPrefix = `[System: File Path: ${msgWithContext.pdfContext.filePath}]\n\n`;
          console.log('[Smart Search] Found PDF context in messages:', msgWithContext.pdfContext.filePath);
          break;
        }
      }

      // If not found in messages, also check storage directly
      if (!pdfContextPrefix) {
        const chatFromStorage = getChat(chatId);
        if (chatFromStorage?.messages) {
          for (const msg of chatFromStorage.messages) {
            if (msg.pdfContext?.filePath) {
              pdfContextPrefix = `[System: File Path: ${msg.pdfContext.filePath}]\n\n`;
              console.log('[Smart Search] Found PDF context in storage:', msg.pdfContext.filePath);
              break;
            }
          }
        }
      ...
      // Prepend PDF context to current API content if we have it and it's not already there
      let finalApiContent = apiUserContent;
      if (pdfContextPrefix && typeof apiUserContent === 'string' && !apiUserContent.includes('[System: File Path:')) {
        finalApiContent = pdfContextPrefix + apiUserContent;
      }
      ...
      const apiMessagesForApi = [...compressedHistory.map(m => ({
        ...
        role: 'user' as const,
        content: finalApiContent, // Use API content with PDF context
      }];
      ...
      // Inject Personalization Settings (Nickname, Occupation, Custom Instructions, Personality)
      let personalizationContent = '';

      // 1. Identity & Tone (Personality)
      if (personality && personality !== 'default') {
        const personalityMap: Record<string, string> = {
          'professional': 'Adopt a professional, formal, and objective tone. Avoid slang or overly casual language. Be precise and polished.',
          'friendly': 'Be warm, enthusiastic, and friendly. You may use emojis occasionally to seem more approachable and chatty.',
          'candid': 'Be direct, honest, and encouraging. Don\'t sugarcoat things, but remain supportive.',
          'quirky': 'Be playful, imaginative, and slightly unpredictable. Use creative metaphors and a fun tone.',
          'efficient': 'Be extremely concise and plain. Avoid conversational filler, pleasantries, or detailed explanations unless asked.',
          'nerdy': 'Be exploratory, enthusiastic about details, and intellectually curious. Use technical terms where appropriate.',
          'cynical': 'Adopt a slightly critical, skeptical, and dryly sarcastic tone. Question assumptions.',
        };
        const pPrompt = personalityMap[personality];
        if (pPrompt) {
          personalizationContent += `\n\n[Personality Mode: ${personality}]\n${pPrompt}`;
        }
      }

      // 2. User Context (Nickname, Occupation, More About You)
      const userContextParts = [];
      if (nickname) userContextParts.push(`- Address the user as: ${nickname}`);
      if (occupation) userContextParts.push(`- User's Occupation: ${occupation}`);
      if (moreAboutYou) userContextParts.push(`- Context about the user: ${moreAboutYou}`);

      if (userContextParts.length > 0) {
        personalizationContent += `\n\n[User Context]\nThe user has provided the following details about themselves to help you personalize your response:\n${userContextParts.join('\n')}`;
      }

      // 3. Custom Instructions
      if (customInstruction) {
        personalizationContent += `\n\n[Custom Instructions]\nThe user has defined the following global instructions for all responses:\n${customInstruction}`;
      }

      // 4. Inject if content exists
      if (personalizationContent) {
        systemMessages.push({
          role: 'system',
          content: `### PERSONALIZATION SETTINGS ###\nThe following settings are enabled to customize your behavior. Prioritize these instructions over default behaviors where applicable.\n${personalizationContent.trim()}`
        });
      }
      ...
      // Server-side auto mode: let the server decide the best model
      // When auto-switch is enabled, call /api/auto-switch first to get the model ID
      // Then update UI and make the chat request with that specific model
      const useAutoMode = isAutoSwitchEnabled();
      let modelToUse = selectedModel;

      if (useAutoMode) {
        console.log(`[Model Selection] Auto mode enabled, calling /api/auto-switch`);
        try {
          const userText = apiMessages.find(m => m.role === 'user')?.content;
          const textInput = typeof userText === 'string' ? userText : '';
          const hasImages = apiMessages.some(msg =>
            Array.isArray(msg.content) && msg.content.some((p: any) => p.type === 'image_url')
          );

          // Call auto-switch to get the best model
          const autoSwitchRes = await fetch('https://ai.collegebuzz.in/api/auto-switch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              input: textInput.slice(0, 500),
              hasImages,
              conversationLength: apiMessages.length,
          ...
          if (autoSwitchRes.ok) {
            const autoSwitch = await autoSwitchRes.json();
            if (autoSwitch.modelId) {
              modelToUse = autoSwitch.modelId;
              console.log(`[Auto Mode] Server selected: ${modelToUse}`);
          ...
          console.warn('[Auto Mode] Auto-switch failed, using current model:', autoErr);
        }
      }

      console.log(`[Model Selection] Using model: ${modelToUse}`);

      // CRITICAL: Detect PDF context for Smart Search tool support
      // The Smart Search tool ONLY works with Cerebras (handleCerebrasStream)
      // Check both message content and pdfContextPrefix (from stored metadata)
      const hasPdfContext = pdfContextPrefix || apiMessages.some(msg => {
        const content = typeof msg.content === 'string' ? msg.content : '';
        return content.includes('[System: File Path:');
      });

      // When PDFs are present, call auto-switch with hasPdf=true to get a Cerebras model
      if (hasPdfContext) {
        console.log('[Smart Search] PDF detected - calling auto-switch for Cerebras model');
        try {
          const userText = apiMessages.find(m => m.role === 'user')?.content;
          const textInput = typeof userText === 'string' ? userText : '';

          // Call the auto-switch endpoint with hasPdf=true
          const autoSwitchRes = await fetch('https://ai.collegebuzz.in/api/auto-switch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              input: textInput.slice(0, 500), // Just the user text for intent detection
              hasImages: false,
              conversationLength: apiMessages.length,
              currentModel: selectedModel,
              hasPdf: true, // CRITICAL: This forces Cerebras selection
              uid: user?.uid,
              // Server will determine tier from uid
            }),
          });
          ...
          if (autoSwitchRes.ok) {
            ...
            if (autoSwitch.modelId) {
              modelToUse = autoSwitch.modelId;
              console.log(`[Smart Search] Auto-switch returned Cerebras model: ${modelToUse}`);
            }
          }
        } catch (autoErr) {
          console.warn('[Smart Search] Auto-switch failed, will try with default:', autoErr);
        }
      }

      await retryWithBackoff(async () => {
        await unifiedStreamCompletion({
          model: modelToUse,
          messages: apiMessages,
          // CRITICAL: These enable Smart Search tool on server
          uid: user?.uid,
          chatId: chatId,
          // Server will determine tier from uid
          onDelta: (delta) => {
          ...
          // Handle Smart Search progress updates ("Reading lines 1-100...")
          onStatus: (status) => {
            console.log('[Smart Search] Status update:', status);
            // Append status to streaming text for visibility
            const statusLine = `\n\n> *${status}*\n\n`;
            fullResponse += statusLine;
            throttledUpdate(fullResponse);
          },
      ...
      // Auto-focus input after response is done
      setTimeout(() => {
  ...
  // Function to get model icon path based on provider, inference, or model name/id
  const getModelIconPath = (provider?: string, modelId?: string, modelLabel?: string): string => {
    const providerLower = (provider || '').toLowerCase();
    const idLower = (modelId || '').toLowerCase();
    const labelLower = (modelLabel || '').toLowerCase();

    // Check for Z.ai/Zai patterns in model name or ID first
    if (idLower.includes('z.ai') || idLower.includes('zai') || labelLower.includes('z.ai') || labelLower.includes('zai')) {
      return '/assets/images/zai.png';
    }

    const iconMap: Record<string, string> = {
      'groq': '/assets/images/groq.png',
      'openai': '/assets/images/openai.png',
      'google': '/assets/images/gemini.png',
      'gemini': '/assets/images/gemini.png',
      'mistral': '/assets/images/mistral.png',
      'cerebras': '/assets/images/zai.png',
      'z.ai': '/assets/images/zai.png',
      'deepseek': '/assets/images/deepseek.png',
      'grok': '/assets/images/grok.png',
      'moonshot': '/assets/images/moonshot.png',
      'qwen': '/assets/images/qwen.png',
      'meta': '/assets/images/meta.png',
      'anthropic': '/assets/images/openai.png', // TODO: Get Anthropic logo
      'openrouter': '/assets/images/openai.png', // OpenRouter is inference, not provider
      'allenai': '/assets/images/allenai.png',
      'arceeai': '/assets/images/arceeai.png',
      'nvidia': '/assets/images/nvidia.png',
      'sourceful': '/assets/images/sourceful.png',
      'venice': '/assets/images/venice.png',
      'xiaomi': '/assets/images/xiaomi.png',
    };

    // Try direct match first
    if (providerLower && iconMap[providerLower]) return iconMap[providerLower];

    // Try with provider name as filename (normalize dots and spaces)
    const normalized = providerLower.replace(/\./g, '').replace(/\s+/g, '');
    if (iconMap[normalized]) return iconMap[normalized];

    // Fallback to groq
    return '/assets/images/groq.png';
  };
  ...
  const [hoveredModelIconMsgId, setHoveredModelIconMsgId] = useState<string | null>(null);
  ...
  const handleShareChat = async (chatId: string) => {
    ...
    try { await navigator.clipboard.writeText(text); toast.success('Chat copied to clipboard'); } catch { toast.error('Failed to copy'); }
  };
  ...
  const streamResponse = async (messageHistory: Message[], selectedEntry: any) => {
    ...
    try {
      ...
      // Use unified API - server handles provider routing automatically
      const useAutoMode = isAutoSwitchEnabled();
      const modelToUse = useAutoMode ? 'auto' : selectedModel;
    ...
    <div
      ...
      {isDragging && (
        <div style={{
          ...
          pointerEvents: 'none', // Allow drop to pass through to container if needed, but since container handles it, this is fine
        }}>
          <div style={{
            padding: '40px',
            borderRadius: '24px',
      ...
      <div style={{
        ...
        {sidebarOpen && (
          <>
            ...
            {/* Account Section */}
            <div style={{ padding: '8px', borderTop: `1px solid ${theme.colors.border}` }}>
              <button
                onClick={() => setShowSettingsModal(true)}
                style={{
                  display: 'flex',
              ...
              >
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', border: `1px solid ${theme.colors.border}`, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: avatarBgColor || '#1f2937' }}>
                  {avatarBgColor ? (
                    <span style={{ fontSize: '14px', fontWeight: 400, color: '#e5e7eb' }}>
                      {initials}
                    </span>
                  ) : null}
                </div>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              {selectedModelObj && (
                <img
                  src={getModelIconPath(selectedModelObj.provider || selectedModelObj.inference, selectedModelObj.id, selectedModelObj.label)}
                  alt={selectedModelObj.label}
                  ...
                  {(() => {
                    ...
                    const Card = ({ model }: { model: CatalogEntry }) => {
                      ...
                      return (
                        ...
                        >
                          <img
                            src={getModelIconPath(model.provider || model.inference, model.id, model.label)}
                            alt={model.label}
          ...
          {messages.length === 0 && !streamingText ? (
            ...
            <>
              {messages.map((msg, msgIndex) => {
                ...
                const showActions = msg.role === 'user' ? hoveredMessageId === msg.id : msg.role === 'assistant';
                ...
                return (
                  ...
                  >
                    {/* Show attachments separately ABOVE the message bubble for user messages */}
                    {msg.role === 'user' && msg.attachments && msg.attachments.length > 0 && (
                      <div style={{
                        display: 'grid',
                        gridTemplateColumns: msg.attachments.length === 1
                          ? 'minmax(200px, 300px)'
                    ...
                    <div style={msg.role === 'user' ? {
                      ...
                      {/* Show attachments for assistant messages (if any) - inside bubble */}
                      ...
                      <div style={{
                        ...
                        {msg.role === 'user' ? (
                          ...
                          <>
                            ...
                            {/* Regenerate button for assistant messages - icon only, only on latest response */}
                            ...
                            {msg.model && (
                              <div
                                ...
                                onMouseEnter={() => setHoveredModelIconMsgId(msg.id)}
                                onMouseLeave={() => setHoveredModelIconMsgId(null)}
                              >
                                <div
                                  style={{
                                    width: 18,
                                ...
                                >
                                  <img
                                    src={getModelIconPath(getProviderName(msg.model), msg.model, msg.model)}
                                    alt={msg.model}
                                    style={{
                                      width: '100%',
                                      height: '100%',
                                ...
                                {hoveredModelIconMsgId === msg.id && (
                                  ...
                                  >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                      <span>{getProviderName(msg.model)}</span>
                                      <span style={{ opacity: 0.5 }}>•</span>
                                      <span style={{ opacity: 0.8 }}>{msg.model}</span>
                                    </div>
                                    {/* Tooltip arrow */}
                                    <div
                                      style={{
              ...
              if (isPillMode) {
                return (
                  <div style={{
                    ...
                    {/* Attachment button */}
                    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', marginRight: '14px' }}>
                      <input type="file" id="image-picker" accept="image/*" multiple style={{ display: 'none' }} onChange={handleImagePick} />
                      <input type="file" id="pdf-picker" accept=".pdf,application/pdf" style={{ display: 'none' }} onChange={handlePDFPick} />
                      <button
                    ...
                    {/* Auto Switch Button - Exact from App */}
                    <button
                      onClick={() => {
                    ...
                    >
                      {autoSwitch ? (
                        <>
                          ...
                          <span
                            style={{
                              ...
                              animation: 'shimmerMove 2.5s cubic-bezier(0.4, 0, 0.6, 1) infinite',
                        ...
                        <>
                          <Wand2 size={16} style={{ color: theme.colors.textSecondary, strokeWidth: 2 }} />
                          <span style={{ color: theme.colors.textSecondary, letterSpacing: '0.4px', marginLeft: 6 }}>Auto</span>
                        </>
                      )}
                    </button>
                    ...
                    {sending ? (
                      <button onClick={handleStopGeneration} style={{
                        width: '40px', height: '40px', borderRadius: '50%', background: theme.colors.text, border: 'none',
                    ...
                    <div style={{
                      ...
                      {attachedImages.map((img, index) => (
                        <div key={`img-${index}`} style={{
                          ...
                          {/* Image */}
                          <img src={img.dataUrl} alt="attachment" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ...
                      {attachedPDFs.map((pdf, index) => (
                        <div key={`pdf-${index}`} style={{
                          ...
                          <div style={{
                            ...
                            display: 'flex', // Fixed syntax error on this line
                            alignItems: 'center',
                            justifyContent: 'center',
                            flexShrink: 0
                          }}>
                            <FileText size={20} color="#ff4444" />
                          </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        <input type="file" id="pdf-picker-rect" accept=".pdf,application/pdf" style={{ display: 'none' }} onChange={handlePDFPick} />
                        <button
                          onClick={() => setShowAttachMenu(!showAttachMenu)}
                          title="Attach files"
                          style={{
                      ...
                      >
                        {autoSwitch ? (
                          ...
                          <>
                            <Wand2 size={16} style={{ color: theme.colors.textSecondary, strokeWidth: 2 }} />
                            <span style={{ color: theme.colors.textSecondary, letterSpacing: '0.4px' }}>Auto</span>
                          </>
                        )}
                      </button>
                    </div>
          ...
          >
            ...
            >
              ...
              <div
                ...
                onClick={() => window.open('https://play.google.com/store/apps/details?id=com.vivekgowdas.SwitchAi', '_blank')}
              >
                {playstoreAnimation && (
                  <Lottie
                    lottieRef={playstoreLottieRef}
                    animationData={playstoreAnimation}
                    loop={true}
                    autoplay={true}
                    style={{ width: '100%', maxWidth: 300, height: 110 }}
                  />
                )}
              </div>
            </div>
      ...
      <SettingsModal
        ...
        setAiMemoryEnabled={setAiMemoryEnabled}
        chatHistorySearchEnabled={chatHistorySearchEnabled}
        setChatHistorySearchEnabled={setChatHistorySearchEnabled}
        ...
        setMoreAboutYou={setMoreAboutYou}
        searchQuery={searchQuery}
        setSearchQuery={setSearchQuery}
        models={models}
        modelsLoading={modelsLoading}
        modelsSortBy={modelsSortBy}
        setModelsSortBy={setModelsSortBy}
